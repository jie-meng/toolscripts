...
## Corpus Check
- 162 files · ~82,393 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1297 nodes · 3226 edges · 94 communities (77 shown, 17 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 45 edges (avg confidence: 0.6)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9da9bfe4`
...
- `main()` --calls--> `add_logging_flags()`  [INFERRED]
  src/toolscripts/commands/ai/ai_links.py → src/toolscripts/core/log.py
- `main()` --calls--> `configure_from_args()`  [INFERRED]
  src/toolscripts/commands/ai/ai_links.py → src/toolscripts/core/log.py
...
### Community 0 - "ask"
Cohesion: 0.08
Nodes (40): get_device_model(), list_devices(), ADB device discovery and selection. Used by every ``android-*`` command. Wraps…, Return the serials of all currently connected ADB devices., Return a single connected device serial, prompting the user if needed. Exits…, Return the human-readable model name for an ADB device serial., select_device(), ADB helpers shared by android-* commands. (+32 more)

### Community 1 - "record_audio.py"
Cohesion: 0.05
Nodes (61): Event, main(), ``android-studio`` - open the current directory in Android Studio (macOS)., main(), ``ios-deeplink`` - launch a deeplink URL on a booted iOS simulator. Requires…, _display_width(), main(), ``ios-log`` - interactive curses-based log viewer for iOS devices. Provides a… (+53 more)

### Community 2 - "ui_curses.py"
Cohesion: 0.06
Nodes (50): NamedTuple, _pick_avd(), Show a curses picker with toggle options. Returns (index, writable, detach) or…, CommandInfo, _discover(), _domain_of(), _filter(), _group_by_domain() (+42 more)
...
Cohesion: 0.11
Nodes (10): LogEntry, LogViewer, Match, window, Curses-based iOS log viewer with level, text filtering, and search., Return all match objects for search_text in entry's raw line., Move to the next search match, preferring visible matches first., Move to the previous search match, preferring visible matches first. (+2 more)

### Community 8 - "add_logging_flags"
Cohesion: 0.09
Nodes (21): main(), ``dec2bin`` - convert decimal numbers to binary (interactive or one-shot)., main(), ``hex2bin`` - convert hexadecimal numbers to binary., main(), ``uuid`` - generate a random UUID4 and copy it to the clipboard., main(), ``git-alias-setup`` - set up common git aliases (st, co, ci, br, lg). (+13 more)
...
Cohesion: 0.08
Nodes (47): Action, _apply_gitignore_policy(), _detect_graphify_gitignore(), _display_items(), _ensure_curses(), _fmt(), _graph_data(), _graph_html_path() (+39 more)

### Community 11 - "require"
Cohesion: 0.10
Nodes (26): main(), ``android-screencast`` - launch scrcpy for the selected device., _get_booted_simulator(), main(), ``ios-record`` - record video from the booted iOS simulator., main(), ``img-scale`` - scale a single image by a factor (0 < scale < 1) via…, main() (+18 more)
...
Cohesion: 0.11
Nodes (21): main(), ``android-adbsync`` - remount the device, sync, and restart the framework., main(), ``dec2hex`` - convert decimal numbers to hexadecimal., main(), ``hex2dec`` - convert hexadecimal numbers to decimal., main(), ``oauth-code`` - generate a TOTP code via ``oathtool`` and copy to clipboard.… (+13 more)
...
Cohesion: 0.08
Nodes (43): main(), ``convert-oneline`` - join all lines of a file into a single line and copy it., _keyword_pattern(), main(), mask_text(), ``redact-clipboard`` - mask credentials/secrets in clipboard text., Mask sensitive values; returns (masked_text, replacement_count)., _ask_prompt_type() (+35 more)
...
### Community 28 - "ccswitch.py"
Cohesion: 0.22
Nodes (10): LogRecord, _build_custom_env(), _detect_current(), _get_or_input(), main(), ``ccswitch`` - switch Claude Code's ~/.claude/settings.json between provider…, _select_provider(), _update_settings() (+2 more)
...
Cohesion: 0.18
Nodes (11): _codebuddy_bin(), main(), ``cbdo`` - run a prompt via CodeBuddy (`cbc`) in print mode with the free hy3…, _extract(), _fetch(), main(), ``free-models-nvidia`` - scrape build.nvidia.com free endpoint models., main() (+3 more)
...
Cohesion: 0.19
Nodes (10): Handler, main(), ``pdf-merge`` - merge all PDF files in a directory into a single result.pdf., _ensure_handler(), Unified colored logger for toolscripts. Quick start:: from toolscripts.core.log…, Set the root toolscripts logger level., Logger subclass exposing a ``success()`` convenience method., _resolve_level() (+2 more)
...
### Community 34 - "rename_project.py"
Cohesion: 0.42
Nodes (9): _is_binary(), _is_ignored(), main(), Path, ``android-rename-project`` - rename an Android project's package across files…, _rreplace(), _update_dir_tree(), _walk_dirs() (+1 more)
...
Cohesion: 0.09
Nodes (30): main(), ``android-keystore-generate`` - wrap keytool to generate Android keystores., main(), ``git-delete-local-branches`` - delete all non-current local branches., _run(), _config_get(), _do_read(), _do_write() (+22 more)
...
Cohesion: 0.25
Nodes (11): main(), ``android-emulator`` - list available AVDs and start the chosen one., _delete_local(), _delete_remote(), _fetch_and_prune(), _list_local_branches(), _list_remote_branches(), main() (+3 more)

### Community 44 - "json_format.py"
Cohesion: 0.60
Nodes (4): format_file(), main(), Path, ``json-format`` - read a JSON file, pretty-print it, save to ``*_format.<ext>``.
...
Cohesion: 0.18
Nodes (14): main(), ``android-record`` - record the screen on an Android device, pull the file,…, compress_video(), Path, Shared helper for compressing video files via ffmpeg. Used by ``android-…, Compress ``input_file`` with ffmpeg. Returns the output path on success., _play(), play_main() (+6 more)
...
- **Why does `configure_from_args()` connect `configure_from_args` to `ask`, `record_audio.py`, `ui_curses.py`, `quick_commit.py`, `run`, `add_logging_flags`, `axios_audit.py`, `graphifycmd.py`, `require`, `trim_audio.py`, `select_many`, `ai_links.py`, `npm_tools.py`, `copy_to_clipboard`, `mongo.py`, `_ffmpeg_quality.py`, `img_resize.py`, `free_models_openrouter.py`, `ccswitch.py`, `checkspace.py`, `get_logger`, `text2num.py`, `core/log.py`, `decode_format_json.py`, `rename_project.py`, `imgcat.py`, `prompts.py`, `extract_games.py`, `merge_to_main.py`, `user.py`, `capture`, `json_format.py`, `playsound.py`, `from_date.py`, `hex2rgb.py`, `list_include.py`, `ocdo.py`, `basic_auth.py`, `pem_to_oneline.py`, `dotnet_globaljson.py`, `statcounter.py`, `web2md.py`, `sensitive_hook.py`, `to_date.py`, `remove_watermark.py`?**
  _High betweenness centrality (0.136) - this node is a cross-community bridge._
- **Why does `add_logging_flags()` connect `add_logging_flags` to `ask`, `record_audio.py`, `ui_curses.py`, `quick_commit.py`, `run`, `axios_audit.py`, `graphifycmd.py`, `require`, `trim_audio.py`, `select_many`, `ai_links.py`, `configure_from_args`, `npm_tools.py`, `copy_to_clipboard`, `mongo.py`, `_ffmpeg_quality.py`, `img_resize.py`, `free_models_openrouter.py`, `ccswitch.py`, `checkspace.py`, `get_logger`, `text2num.py`, `core/log.py`, `decode_format_json.py`, `rename_project.py`, `imgcat.py`, `prompts.py`, `extract_games.py`, `merge_to_main.py`, `user.py`, `capture`, `json_format.py`, `playsound.py`, `from_date.py`, `hex2rgb.py`, `list_include.py`, `ocdo.py`, `basic_auth.py`, `pem_to_oneline.py`, `dotnet_globaljson.py`, `statcounter.py`, `web2md.py`, `sensitive_hook.py`, `to_date.py`, `remove_watermark.py`?**
  _High betweenness centrality (0.135) - this node is a cross-community bridge._
- **Why does `get_logger()` connect `get_logger` to `ask`, `record_audio.py`, `ui_curses.py`, `quick_commit.py`, `run`, `add_logging_flags`, `axios_audit.py`, `require`, `trim_audio.py`, `select_many`, `configure_from_args`, `npm_tools.py`, `ios/devices.py`, `copy_to_clipboard`, `mongo.py`, `_ffmpeg_quality.py`, `img_resize.py`, `free_models_openrouter.py`, `ccswitch.py`, `checkspace.py`, `text2num.py`, `core/log.py`, `decode_format_json.py`, `rename_project.py`, `imgcat.py`, `prompts.py`, `extract_games.py`, `merge_to_main.py`, `user.py`, `capture`, `json_format.py`, `playsound.py`, `from_date.py`, `hex2rgb.py`, `list_include.py`, `ocdo.py`, `basic_auth.py`, `pem_to_oneline.py`, `dotnet_globaljson.py`, `statcounter.py`, `web2md.py`, `sensitive_hook.py`, `to_date.py`, `remove_watermark.py`?**
...
- **Should `ask` be split into smaller, more focused modules?**
