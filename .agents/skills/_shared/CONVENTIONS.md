...
| Read npm global packages (skips node-bundled `npm`/`corepack` and npm's `.`-prefixed trash dirs) | `read_global_packages(modules_dir)` → `dict[str, str]` | `toolscripts.core.npm_global` |
