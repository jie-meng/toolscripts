...
def main():
    ...
    if not udid:
        ...
        print("Start a simulator first: xcrun simctl boot <device-udid>", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        for line in record_process.stderr:
            ...
            if record_process.poll() is not None:
                ...
                print(f"{RED}Error: simctl exited without starting recording.{NC}", file=sys.stderr)
                sys.exit(1)
