...
| `llama`      | `huggingface-hub`                             | `llamacmd` (HF search / download)  |
...
| git        | `git-alias-setup`, `git-copy-diff`, `git-branch-delete`, `git-merge-to-main`, `git-delete-local-branches`, `git-make-patches`, `git-apply-patches`, `git-install-sensitive-hook`, `git-user`, `git-user-batch`, `git-quick-commit` |
...
| ai         | `ocdo`, `ocdo-models`, `cbdo`, `pido`, `free-models-openrouter`, `free-models-nvidia`, `agents-setup`, `agents-cleanup`, `ai-links`, `graphifycmd`, `npm-tools`, `llamacmd`, `pi-extensions-setup` |
| text/docs  | `markdown-snippet`, `slugify`, `web2md`, `translate`, `mermaid`, `mmdcmd`, `statcounter-os-coverage`, `xlsx-text2num`, `docs-pick` |
| system     | `dotnet-globaljson`, `toolscripts-list`, `myip`, `checkspace`, `lsdevcu`, `rm-ds-store`, `rm-meta`, `dirdiff`, `intellij`, `pycharm`, `xcode`, `venv-create`, `uvcmd`, `iterm-setup`, `codebuddy-setup`, `kill-pwchrome`, `brew-upgrade`, `npm-gsync`, `pi-update-all`, `rpi-vpn-setup` |
